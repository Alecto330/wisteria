...
function handleFileChange(event) {
    ...
    if (file && file.type === 'application/pdf') {
        ...
        // Mostra alert di errore se il file non è un PDF
...
function aggiungiEsperienza() {
    ...
    fetch('/wisteria/profilepage', {
        ...
        body: formData.toString() // Serializza il formData
    })
    .then(response => {
        if (response.redirected) {
            // Redirigi se il server lo richiede
...
function showAlert(message) {
    ...
    alertBox.textContent = message; // Imposta il messaggio dell'alert
